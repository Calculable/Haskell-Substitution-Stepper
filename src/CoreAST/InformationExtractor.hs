...
module CoreAST.InformationExtractor
  ( varToString,
    nameToString,
    isTypeInformation,
    canBeReduced,
    ...
    varNameEqualsString,
    varsHaveTheSameType,
    isApplicationWithClassDictionary,
    functionNameMatchesFunctionFromDictionary,
    ...
    isOperator,
    varToSimpleString,
    getClassDictionaryVar,
  )
where

import CoreAST.TypeDefinitions (FunctionReference)
import Data.List (isPrefixOf, isSuffixOf)
import GHC.Core.TyCo.Rep (Type)
import GHC.Plugins
  ( Bind (NonRec, Rec),
    CoreExpr,
    Expr (App, Case, Cast, Lam, Let, Lit, Type, Var),
    Name,
    Type,
    Var (varName, varType),
    collectArgs,
...
isListType ty = "[" `isPrefixOf` typeRepresentation && "]" `isSuffixOf` typeRepresentation
  where
    typeRepresentation = showOutputable ty
