...
data ExpressionS
    ...
    | MultiArgumentAppS {name:: String, argumentsS :: [ExpressionS]} --not in original core but used so we can make a reduction with built-in functions/operators from the prelude like (+ 1) 2 -> (+ 1 2) -> 3 
